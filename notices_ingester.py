...
def get_notice_files_recursive(folder_id, folder_name="Notices Root"):
    files_list = []
    page_token = None
    ...
    while True:
        ...
        for item in response.get('files', []):
            ...
            if mime_type == 'application/vnd.google-apps.folder':
                files_list.extend(get_notice_files_recursive(item.get('id'), item.get('name')))
            elif mime_type in [
                'application/pdf', 
...
def process_notice_content(full_text, file_name):
    doc_title = file_name
    chunks = []
    
    # التقطيع بناءً على أقسام الإنذار العدلي/القانوني
    pattern = r'\n(?=\s*(?:المنذر|المنذر إليه|الموضوع|الوقائع|بناء عليه|لذلك|الطلبات|تفاصيل الإنذار))'
    ...
    if len(sections) <= 2:
        ...
        for i in range(0, len(words), chunk_size - 40):
            chunks.append({
                ...
                "metadata": {"document_name": doc_title, "document_type": "إنذار قانوني", "source_file": file_name, "chunk_index": i}
            })
        ...
        if len(section_text) > 30:
            chunks.append({
                ...
                "metadata": {
                    ...
                    "document_type": "إنذار قانوني (قسم مستقل)",
...
def ingest_notice_chunks(chunks_data):
    if not chunks_data:
        return

    doc_name = chunks_data[0]["metadata"]["document_name"]
    print(f"  -> Uploading {len(chunks_data)} sections to legal_notices for: {doc_name}...")
    ...
    for chunk_info in chunks_data:
        ...
        try:
            supabase.table("legal_notices").insert(data).execute()
...
if __name__ == "__main__":
    ...
    for file_info in files_to_process:
        ...
        try:
            existing = supabase.table("legal_notices").select("id").eq("source_file", file_name).limit(1).execute()
            if existing.data:
            ...
            extracted_chunks, _ = process_notice_content(extracted_text, file_name)
            
            if extracted_chunks:
                ingest_notice_chunks(extracted_chunks)
            else:
    ...
    print("\n--- ALL NOTICES PROCESSED SUCCESSFULLY! ---")